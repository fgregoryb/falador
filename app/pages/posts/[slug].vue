  ...
  <div>
    ...
    <article v-else-if="post" style="max-width:var(--maxw);margin:0 auto;padding:0 32px;position:relative;">
      <div style="max-width:720px;margin:0 auto;padding:56px 0 0;">
        <div class="rise">
          <h1 class="serif" style="font-size:clamp(34px,4.4vw,46px);font-weight:500;line-height:1.1;letter-spacing:-0.025em;margin:0;color:var(--text);">
            {{ post.title }}
          </h1>
          ...
          <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;margin:28px 0 0;padding-bottom:30px;border-bottom:1px solid var(--border);">
            <div style="display:flex;align-items:center;gap:12px;">
              <!-- Avatar dinâmico -->
              <div style="width:42px;height:42px;border-radius:50%;overflow:hidden;background:var(--accent-weak);display:grid;place-items:center;flex:none;">
                <img v-if="profile?.avatar_url" :src="profile.avatar_url" :alt="profile.name"
                  style="width:100%;height:100%;object-fit:cover;" />
                <span v-else style="color:var(--accent);font-family:var(--font-display);font-weight:600;font-size:18px;">
                  {{ (profile?.name || 'A').charAt(0).toUpperCase() }}
                </span>
              </div>
              <div>
                <div style="font-weight:600;font-size:14.5px;">{{ profile?.name ?? 'Autor' }}</div>
        ...
        <img v-if="(post as any).show_cover !== false"
          :src="(post as any).cover_image_url || coverImage(post.slug, 1200, 600)" :alt="post.title"
          class="fade"
        ...
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;margin:48px 0 0;padding:28px 0;border-top:1px solid var(--border);border-bottom:1px solid var(--border);">
          ...
          <div style="display:flex;gap:8px;">
            <a v-if="settings?.twitter_url" :href="settings.twitter_url" target="_blank" rel="noreferrer" class="btn btn-ghost btn-icon"><FIcon name="twitter" :size="16" /></a>
            <a v-if="settings?.linkedin_url" :href="settings.linkedin_url" target="_blank" rel="noreferrer" class="btn btn-ghost btn-icon"><FIcon name="linkedin" :size="16" /></a>
        ...
        <div class="card" style="display:flex;gap:18px;padding:24px;margin:34px 0 60px;align-items:flex-start;">
          <div style="width:56px;height:56px;border-radius:50%;overflow:hidden;background:var(--accent-weak);display:grid;place-items:center;flex:none;">
            <img v-if="profile?.avatar_url" :src="profile.avatar_url" :alt="profile?.name"
              style="width:100%;height:100%;object-fit:cover;" />
            <span v-else style="color:var(--accent);font-family:var(--font-display);font-weight:600;font-size:22px;">
              {{ (profile?.name || 'A').charAt(0).toUpperCase() }}
            </span>
          </div>
          <div style="flex:1;">
            <div class="eyebrow" style="margin-bottom:6px;">Escrito por</div>
            <div style="font-weight:600;font-size:16px;font-family:var(--font-display);">{{ profile?.name ?? 'Autor' }}</div>
...
const [{ data: post, pending, error }, { data: profile }, { data: settings }] = await Promise.all([
  useFetch(`/api/posts/slug/${route.params.slug}`),
